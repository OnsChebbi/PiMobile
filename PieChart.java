...
public class PieChart extends Form {
    ...
    public Component execute(int v1, int v2, int v3,int v4,int v5) {
        double[] values = new double[]{v1, v2, v3, v4, v5};
        ...
        final CategorySeries seriesSet = buildCategoryDataset("nbre de cours par liste", values);
    ...
    public PieChart() {
        setTitle("Les Domaines Artistiques");
    ...
    //    ComboBox classes = new ComboBox();
       ...
       // ArrayList<Classe> arr = new ArrayList<>();
            ...
            chart.removeAll();
            //removeComponent(sp);
            ArrayList<User> list_e = new ServiceUser().all();
                    
    int v1=0,v2=0,v3=0,v4=0,v5=0;
    for(User cu : list_e )
    {
        if (cu.getDomaine().equals("Peinture"))
        {
            v1++;
        }
        if (cu.getDomaine().equals("Musique"))
        {
            v2++;
        }
        if (cu.getDomaine().equals("Cuisine"))
        {
            v3++;
        }
        if (cu.getDomaine().equals("Photographie"))
        {
            v4++;
        }
        if (cu.getDomaine().equals("Cinéma"))
        {
            v5++;
        }
    }

                  sp = execute(v1, v2, v3, v4, v5);
                //   sp = execute(15, 20, 30);
                    chart.add(sp);
